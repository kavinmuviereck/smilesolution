...
if (isset($_POST['update'])) {
    ...
    $rest   = $db->updateAry("conferences", $random, "WHERE id='" . $_POST['tableid'] . "'");
        ...
        if (isset($_FILES['imagefiles']['name'][$i]) && $_FILES['imagefiles']['name'][$i] != '' && !empty($_FILES['imagefiles']['name'][$i])) {
            ...
            if (in_array($ext, array('jpeg','jpg','gif','png'))) {
                ...
                if (move_uploaded_file($_FILES['imagefiles']['tmp_name'][$i], '../images/services/' . $newfile)) {
                    ...
                    $rests = $db->insertAry('conferences', $imgs);
                }
            } else {
                $stat["error"] = "Please Select valid file";
            }
            ...
            <div class="container-fluid pdt-20">
                ...
                <div class="row panel panel-default mgt-20">
                    <div class="panel-heading">
                        <h1 class="white-text page-heading text-center">Conferences</h1>
                    </div>
                                ...
                                <div class="panel-body">
                                    <table width="100%" class="table table-bordered table-striped table-hover" id="dataTables-example-des-des">
                                        ...
                                        <tbody>
                                            <?php
                                            $counter = 0;
                                            $clients = $db->getRows("SELECT * from conferences order by id desc");
                                            if (count($clients) > 0) {
                                            ...
                                            ?>
                                                    <tr class="odd gradeX">
                                                        ...
                                                        <td class="text-center ac_projects_delete">
                                                            <button type="button" id="delete" onclick="DeleteConfirmation(this,'conferences')" name="delete" value="<?= $id ?>" class="btn btn-danger btn-circle"><i class="fa fa-times"></i>
                                                            </button>
                                                        </td>
                                                    </tr>
    ...
    <script>
        ...
        $(document).on("click", ".btnCategoryEdit", function(event) {
            ...
            $.ajax({
                    url: 'ajax.php',
                    type: 'POST',
                    data: 'action=edit_conferences&Id=' + value,
                    dataType: 'html'
                })
                .done(function(data) {
                    console.log(data);
